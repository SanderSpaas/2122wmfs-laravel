    ...
    <main class="container">
        @if ($blogposts)
            <div class="p-4 p-md-5 mb-4 text-white rounded bg-dark banner"
                ...
                <p class="lead mb-0"><a href="{{ url('blogpost/' . $blogposts[0]->id) }}"
                    ...
                    <div class="col-md-6">
                        <div
                            ...
                            <div class="col p-4 d-flex flex-column position-static">
                                <strong
                                    class="d-inline-block mb-2 category-2">{{ $blogposts[$i]->category->title }}</strong>
                                <h3 class="mb-0">{{ $blogposts[$i]->title }}</h3>
                                <div class="mb-1 text-muted">{{ $blogposts[$i]->created_at }} @foreach ($blogposts[$i]->tags as $tag)
                                        <span class="badge bg-success">{{ $tag->title }}</span>
                                    @endforeach
                                </div>
                                <p class="card-text mb-auto">{{ $blogposts[$i]->content }}</p>
                                <a href="{{ url('blogpost/' . $blogposts[$i]->id) }}" class="stretched-link">Continue
                                    reading</a>
                            </div>
                            <div class="col-auto d-none d-lg-block img-container">
                                <img src="{{ url('storage/' . $blogposts[$i]->image) }}"
